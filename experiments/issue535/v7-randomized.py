...
REVISIONS = ["issue535-base", "issue535-v7"]
CONFIGS = [
    IssueConfig(
        "lazy-ff-pref-{pref_first}-randomize-{randomize}-seed-{seed}".format(**locals()),
        ["--heuristic", "hff=ff()", "--random-seed", str(seed), "--search",
         "lazy_greedy(hff, preferred_successors_first={pref_first}, randomize_successors={randomize}, preferred=hff)".format(**locals())])
    for pref_first in [True]
...
exp.add_scatter_plot_step(relative=True, attributes=["total_time", "evaluations"])
