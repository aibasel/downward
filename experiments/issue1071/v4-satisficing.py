...
def make_comparison_table():
    report = common_setup.ComparativeReport(
        algorithm_pairs=[
            (f"{ISSUE}-base-lama-first", f"{ISSUE}-v4-lama-first"),
            (f"{ISSUE}-base-lama-first-pref", f"{ISSUE}-v4-lama-first-pref"),
            (f"{ISSUE}-base-lm-zg", f"{ISSUE}-v4-lm-zg"),
...
CONFIGS = [
    ...
    IssueConfig("lm-zg", ["--search", "eager_greedy([landmark_sum(lm_zg())])"]),
]
...
exp.add_fetcher("data/issue1071-base-satisficing-v4-eval", name="fetch base")
