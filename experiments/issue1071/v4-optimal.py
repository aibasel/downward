...
def make_comparison_table():
    report = common_setup.ComparativeReport(
        algorithm_pairs=[
            (f"{ISSUE}-base-seq-opt-bjolp", f"{ISSUE}-v4-seq-opt-bjolp"),
            (f"{ISSUE}-base-seq-opt-bjolp-opt", f"{ISSUE}-v4-seq-opt-bjolp-opt"),
            (f"{ISSUE}-base-lm-hm2", f"{ISSUE}-v4-lm-hm2"),
            (f"{ISSUE}-base-lm-exhaust", f"{ISSUE}-v4-lm-exhaust"),
...
CONFIGS = [
    ...
    IssueConfig("lm-hm2",
                ["--evaluator", "lmc=landmark_cost_partitioning(lm_hm(m=2))",
                 "--search", "astar(lmc,lazy_evaluator=lmc)"]),
    IssueConfig("seq-opt-bjolp", [],
                driver_options=["--alias", "seq-opt-bjolp"]),
    IssueConfig("seq-opt-bjolp-opt",
                ["--evaluator",
                 "lmc=landmark_cost_partitioning(lm_merged([lm_rhw(),lm_hm(m=1)]), optimal=true)",
                 "--search", "astar(lmc,lazy_evaluator=lmc)"]),
]
...
exp.add_fetcher("data/issue1071-base-optimal-v4-eval", name="fetch base")
