...
public:
    ...
    virtual std::shared_ptr<BestFirstOpenListFactory> create_task_specific_BestFirstOpenListFactory(const std::shared_ptr<AbstractTask> &task, std::unique_ptr<ComponentMap> &component_map, int depth = -1);
};
}
