...
public:
    explicit NthTypeBasedOpenList(const plugins::Options &opts);
...
template<class Entry>
NthTypeBasedOpenList<Entry>::NthTypeBasedOpenList(const plugins::Options &opts)
...
template<class Entry>
bool NthTypeBasedOpenList<Entry>::empty() {
...
class NthTypeBasedOpenListFeature : public plugins::TypedFeature<OpenListFactory, NthTypeBasedOpenListFactory> {
public:
    NthTypeBasedOpenListFeature(): TypedFeature("nth_type_based"){
	document_title("NthType-based open list");
	document_synopsis(
	...
	add_option<int>("n", "how many h-values to explore", "2");
	add_option<bool>(
	    "ignore_size",
...
static plugins::FeaturePlugin<NthTypeBasedOpenListFeature> _plugin;
}
