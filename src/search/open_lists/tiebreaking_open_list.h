...
public:
    ...
    virtual std::shared_ptr<TieBreakingOpenListFactory> create_task_specific_TieBreakingOpenListFactory(const std::shared_ptr<AbstractTask> &task, std::shared_ptr<ComponentMap> &component_map, int depth = -1);
};
}
