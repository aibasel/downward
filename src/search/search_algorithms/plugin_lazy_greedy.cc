...
public:
    LazyGreedySearchFeature() : TypedFeature("lazy_greedy") {
        ...
        document_note(
            ...
            "\n```\n--search let(h2, eval2, lazy_greedy([eval1, h2], preferred=[h2], boost=100))\n```\n"
            "is equivalent to\n"
            "```\n--search let(h1, eval1, let(h2, eval2, lazy(alt([single(h1), single(h1, pref_only=true), single(h2),\n"
            ...
            "------------------------------------------------------------\n"
            "```\n--search let(h1, eval1, lazy_greedy([h1], preferred=[h1]))\n```\n"
            "is equivalent to\n"
            "```\n--search let(h1, eval1, lazy(alt([single(h1), single(h1, pref_only=true)], boost=1000),\n"
            "              preferred=[h1]))\n```\n"
            "------------------------------------------------------------\n"
