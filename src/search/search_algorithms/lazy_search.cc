...
SearchStatus LazySearch::fetch_next_state() {
    ...
    */
    current_eval_context =
        EvaluationContext(current_state, current_g, true, &statistics);
...
SearchStatus LazySearch::step() {
    ...
    // heuristic.
    // - current_predecessor_id is the state ID of the predecessor of that
    // state.
    // - current_operator_id is the ID of the operator which leads to
