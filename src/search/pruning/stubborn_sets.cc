...
bool StubbornSets::can_disable(ActionID op1_id, ActionID op2_id) const {
    return contain_conflicting_fact(sorted_op_effects[op1_id.get_index()],
                                    sorted_op_preconditions[op2_id.get_index()]);
}

// Relies on op_effect being sorted by variable.
bool StubbornSets::can_conflict(ActionID op1_id, ActionID op2_id) const {
    return contain_conflicting_fact(sorted_op_effects[op1_id.get_index()],
                                    sorted_op_effects[op2_id.get_index()]);
}
...
void StubbornSets::compute_achievers(const TaskProxy &task_proxy) {
    achievers = utils::map_vector<vector<vector<ActionID>>>(
        task_proxy.get_variables(), [](const VariableProxy &var) {
            return vector<vector<ActionID>>(var.get_domain_size());
        });

    for (const OperatorProxy op : task_proxy.get_operators()) {
        for (const EffectProxy effect : op.get_effects()) {
            FactPair fact = effect.get_fact().get_pair();
            achievers[fact.var][fact.value].push_back(op.get_action_id());
        }
    }
}

bool StubbornSets::mark_as_stubborn(ActionID op_id) {
    if (!stubborn[op_id.get_index()]) {
        stubborn[op_id.get_index()] = true;
        stubborn_queue.push_back(op_id);
        return true;
    }
    return false;
}
...
void StubbornSets::prune_operators(
    ...
    while (!stubborn_queue.empty()) {
        ActionID op_id = stubborn_queue.back();
        stubborn_queue.pop_back();
        handle_stubborn_operator(state, op_id);
    }
