...
template <>
class TokenParser<TaskInterface *> {
public:
    static inline TaskInterface *parse(OptionParser &p);
};
...
TaskInterface *TokenParser<TaskInterface *>::parse(OptionParser &p) {
    return lookup_in_registry<TaskInterface>(p);
}
