...
pair<int, int> Abstraction::refine(
    ...
    pair<CartesianSet, CartesianSet> cartesian_sets = state->split_domain(var, wanted);

    AbstractState *v1 = new AbstractState(v1_id, node_ids.first, move(cartesian_sets.first));
    AbstractState *v2 = new AbstractState(v2_id, node_ids.second, move(cartesian_sets.second));
