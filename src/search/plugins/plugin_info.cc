...
ArgumentInfo::ArgumentInfo(
    const string &key, const string &help, const Type &type,
    const string &default_value, const Bounds &bounds, bool lazy_construction)
    : key(key),
      help(help),
      type(type),
...
NoteInfo::NoteInfo(
    const string &name, const string &description, bool long_text)
    : name(name), description(description), long_text(long_text) {
}

LanguageSupportInfo::LanguageSupportInfo(
    const string &feature, const string &description)
