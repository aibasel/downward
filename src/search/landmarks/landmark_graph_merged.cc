...
LandmarkNode *LandmarkGraphMerged::get_matching_landmark(const LandmarkNode &lm) const {
    if (!lm.disjunctive && !lm.conjunctive) {
        const FactPair lm_fact(lm.vars[0], lm.vals[0]);
        ...
        for (size_t i = 0; i < lm.vars.size(); ++i) {
            lm_facts.insert(FactPair(lm.vars[i], lm.vals[i]));
        }
        if (lm_graph->exact_same_disj_landmark_exists(lm_facts))
            return &lm_graph->get_disj_lm_node(FactPair(lm.vars[0], lm.vals[0]));
        else
            return 0;
...
void LandmarkGraphMerged::generate_landmarks(const TaskProxy &task_proxy, Exploration &exploration) {
    ...
    for (size_t i = 0; i < lm_graphs.size(); ++i) {
        ...
        for (it = nodes.begin(); it != nodes.end(); ++it) {
            ...
            const FactPair lm_fact(node.vars[0], node.vals[0]);
        ...
        for (it = nodes.begin(); it != nodes.end(); ++it) {
            ...
            if (node.disjunctive) {
                ...
                for (size_t j = 0; j < node.vars.size(); ++j) {
                    const FactPair lm_fact(node.vars[j], node.vals[j]);
