...
public:
    explicit TaskIndependentGEvaluator(utils::LogProxy log,
                                       ...
                                       bool use_for_counting_evaluations = false);
    template<typename T>
    std::shared_ptr<T> create_task_specific(std::shared_ptr<AbstractTask> &task, std::unique_ptr<ComponentMap> &component_map, int depth = -1);
    ...
    virtual std::shared_ptr<GEvaluator> create_task_specific_GEvaluator(const std::shared_ptr<AbstractTask> &task, int depth = -1);
    virtual std::shared_ptr<GEvaluator> create_task_specific_GEvaluator(const std::shared_ptr<AbstractTask> &task, std::unique_ptr<ComponentMap> &component_map, int depth = -1);
};
}
