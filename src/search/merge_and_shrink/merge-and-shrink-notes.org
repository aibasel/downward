...
- Merge strategy base class could have the field remaining_merges rather
  than dfp merge strategy. Also done() could be implemented there. The
  linear merge strategy should then make sure that remaining_merges is
  always synchronized with the variable order finde its using.
