...
int BestFirstSearchEngine::step() {
  ...
  if(!closed.contains(current_state)) {
	...
	if(g_multi_heuristic)
	  open1_expansions -= 1000; // HACK! Helpful action benefit.
	if(best_h == 0) {
  ...
  // Fetch new state.
  if(open1.empty() && open2.empty() && open3.empty() && open4.empty()) {
  ...
  bool use_open1 = 1 * open1_expansions <= 1 * open2_expansions;
  use_open1 = use_open1 && !open1.empty() || open2.empty();
  OpenList &open = use_open1
    ? (use_ff && !open3.empty() ? open3 : open1)
    : (use_ff && !open4.empty() ? open4 : open2);
  if(use_open1)
    open1_expansions++;
  else
    open2_expansions++;
