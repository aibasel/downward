...
class Cache {
  ...
  vector<vector<ValueTransitionLabel *> > helpful_action_cache;
...
public:
  ...
  ValueTransitionLabel *lookup_helpful_action(int var, const State &state,
					      int from_val, int to_val) const {
    return helpful_action_cache[var][get_index(var, state, from_val, to_val)];
  }
  void store_helpful_action(int var, const State &state, int from_val, int to_val,
			    ValueTransitionLabel *helpful_action) {
    helpful_action_cache[var][get_index(var, state, from_val, to_val)] = helpful_action;
  }
};
