...
int main(int argc, const char **argv) {
  ...
  bool found_solution = false;
  {
    ...
    scheduler.search();
    times(&search_end);
    found_solution = scheduler.found_solution();
    if(found_solution)
  ...
  if(!found_solution && !use_climber && !g_multi_heuristic) {
    cout << "failure... but can we be sure?" << endl;
    cout << "better replan with the FF heuristic" << endl;

    // We want to prove unsolvability; no use in helpful actions.
    g_downward_helpful_actions = false;
    g_ff_helpful_actions = false;
    g_mixed_helpful_actions = false;

    g_ff_heuristic_replanning = true;
