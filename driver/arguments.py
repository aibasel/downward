...
"""
def print_usage_and_error_and_exit(parser, msg):
    parser.print_usage()
    ...
    for pos, (name1, is_specified1) in enumerate(args):
        for name2, is_specified2 in args[pos + 1:]:
            if is_specified1 and is_specified2:
                print_usage_and_error_and_exit(parser, "cannot combine %s with %s" % (name1, name2))
    if required and not any(is_specified for _, is_specified in args):
        print_usage_and_error_and_exit(parser, "exactly one of {%s} has to be specified" %
...
def _set_components_and_inputs(parser, args):
    ...
    if first == "translate":
        if "--help" in args.translate_options or "-h" in args.translate_options:
            ...
            print_usage_and_error_and_exit(parser, "translator needs one or two input files")
    elif first == "search":
        if "--help" in args.search_options:
            ...
            print_usage_and_error_and_exit(parser, "search needs exactly one input file")
    else:
        assert False, first
...
def parse_args():
    ...
    if args.build and args.debug:
        print_usage_and_error_and_exit(parser, "The option --debug is an alias for --build=debug32 "
    ...
    if args.alias:
        try:
            ...
            print_usage_and_error_and_exit(parser, "unknown alias: %r" % args.alias)

    if args.portfolio_bound is not None and not args.portfolio:
        print_usage_and_error_and_exit(parser, "--portfolio-bound may only be used for portfolios.")
    if args.portfolio_bound is not None and args.portfolio_bound < 0:
        print_usage_and_error_and_exit(parser, "--portfolio-bound must not be negative.")
    if args.portfolio_single_plan and not args.portfolio:
        print_usage_and_error_and_exit(parser, "--portfolio-single_plan may only be used for portfolios.")
