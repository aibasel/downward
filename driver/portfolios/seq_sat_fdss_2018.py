...
CONFIGS = [
    ...
    (24, ["--search",
        ...
        "let(hblind, blind(),"
        "let(hadd, add(),"
        "let(hlm, lmcount(lmg,admissible=false,pref=true,transform=adapt_costs(plusone)),"
        "let(hff, ff(),"
    ...
    (29, ["--search",
        "let(lmg, lm_hm(conjunctive_landmarks=false,use_orders=false,m=1),"
        "let(hlm, lmcount(lmg,admissible=true),"
        "let(hff, ff(),"
    ...
    (30, ["--search",
        ...
        "let(hhmax, hmax(),"
        "let(hblind, blind(),"
        "let(hlm, lmcount(lmg,admissible=true,pref=false,transform=adapt_costs(one)),"
    ...
    (56, ["--search",
        "let(lmg, lm_exhaust(only_causal_landmarks=false),"
        "let(hgoalcount, goalcount(transform=adapt_costs(plusone)),"
        "let(hlm, lmcount(lmg,admissible=false),"
        "let(hff, ff(),"
        "let(hblind, blind(),"
    ...
    (24, ["--search",
        ...
        "let(hlm, lmcount(lmg,admissible=true,pref=false),"
