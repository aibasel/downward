...
CONFIGS = [
    ...
    (114, ["--search",
         ...
         "let(hlm, lmcount(lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(plusone)),"
    ...
    # add_lm_lazy_wastar
    (79, ["--search",
         "let(hadd, add(transform=H_COST_TRANSFORM)," "let(hlm, lmcount(lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(plusone)),"
          "lazy_wastar([hadd,hlm],w=3,preferred=[hadd,hlm],cost_type=S_COST_TYPE,bound=BOUND)))"]),
    # ff_lm_lazy_wastar
    (159, ["--search",
         "let(hff, ff(transform=H_COST_TRANSFORM)," "let(hlm, lmcount(lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(plusone)),"
          "lazy_wastar([hff,hlm],w=3,preferred=[hff,hlm],cost_type=S_COST_TYPE,bound=BOUND)))"]),
    # cea_lm_lazy_wastar
    (39, ["--search",
         "let(hcea, cea(transform=H_COST_TRANSFORM)," "let(hlm, lmcount(lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(plusone)),"
    ...
    # cea_ff_eager_wastar
    (40, ["--search",
         "let(hff, ff(transform=H_COST_TRANSFORM)," "let(hcea, cea(transform=H_COST_TRANSFORM),"
    ...
    # cg_lm_eager_wastar
    (78, ["--search",
         "let(hcg, cg(transform=H_COST_TRANSFORM)," "let(hlm, lmcount(lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(plusone)),"
