...
CONFIGS = [
    # add_lm_lazy_greedy
    (114, ["--search",
         "let(hadd, cost_adapted_heuristic(add(),cost_type=H_COST_TYPE),"
         "let(hlm, cost_adapted_heuristic(landmark_sum(lm_reasonable_orders_hps(lm_rhw())),cost_type=plusone),"
         "lazy_greedy([hadd,hlm],preferred=[hadd,hlm],cost_type=S_COST_TYPE,bound=BOUND)))"]),
    # ff_lm_lazy_greedy
    (187, ["--search",
         "let(hff, cost_adapted_heuristic(ff(),cost_type=H_COST_TYPE),"
         "let(hlm, cost_adapted_heuristic(landmark_sum(lm_reasonable_orders_hps(lm_rhw())),cost_type=plusone),"
    ...
    # cea_lm_lazy_greedy
    (39, ["--search",
         "let(hcea, cost_adapted_heuristic(cea(),cost_type=H_COST_TYPE),"
         "let(hlm, cost_adapted_heuristic(landmark_sum(lm_reasonable_orders_hps(lm_rhw())),cost_type=plusone),"
    ...
    (40, ["--search",
         "let(hff, cost_adapted_heuristic(ff(),cost_type=H_COST_TYPE),"
         "let(hcg, cost_adapted_heuristic(cg(),cost_type=H_COST_TYPE),"
