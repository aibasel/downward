...
static LandmarksGraph *build_landmarks_graph(Exploration* exploration) {
    ...
    enum {rpg_sasp, zhu_givan, exhaust} landmarks_type = rpg_sasp; // change for different landmarks
...
LandmarksCountHeuristic(bool preferred_ops, bool admissible)
...
int LandmarksCountHeuristic::get_heuristic_value(const State& state) {
    ...
    if (use_shared_cost) {
    	...
    	if (use_action_landmark_count) {
    		...
    		reached_cost    = lgraph.get_not_unused_alm_effect_reached_shared_cost();
    ...
    h = ceil(total_cost - reached_cost + needed_cost + additional_cost - epsilon);
    /*
    if (g_verbose) {
...
check_node_orders_disobeyed(LandmarkNode& node,
...
bool LandmarksCountHeuristic::reach_state(const State& parent_state, const Operator &op,
        		const State& state) {
