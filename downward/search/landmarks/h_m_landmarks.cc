...
template<typename T>
std::ostream &
operator<<(std::ostream &os, const std::list<T> &alist) {
...
template<typename T>
void
union_with(std::list<T> &alist, const std::list<T> &other) {
...
template<typename T>
void
intersect_with(std::list<T> &alist, const std::list<T> &other) {
...
template<typename T>
void
set_minus(std::list<T> &alist, const std::list<T> &other) {
    typename std::list<T>::iterator it1 = alist.begin(), tmp;
    typename std::list<T>::const_iterator it2 = other.begin();
...
template<typename T>
void
insert_into(std::list<T> &alist, const T &val) {
    typename std::list<T>::iterator it1 = alist.begin();
...
template<typename T>
bool
contains(std::list<T> &alist, const T &val) {
    typename std::list<T>::iterator it1 = alist.begin();
...
void
HMLandmarks::get_m_sets_(int m, int num_included, int current_var,
                         FluentSet &current,
...
void
HMLandmarks::get_m_sets_of_set_(int m, int num_included, int current_var_index,
                                FluentSet &current,
                                std::vector<FluentSet > &subsets,
...
void
HMLandmarks::get_split_m_sets_(int m,
...
void
HMLandmarks::get_m_sets(int m, std::vector<FluentSet> &subsets) {
    FluentSet c;
...
void
HMLandmarks::get_m_sets(int m, std::vector<FluentSet> &subsets, const FluentSet &superset) {
    FluentSet c;
...
// assume disjoint
void
HMLandmarks::get_split_m_sets(int m, std::vector<FluentSet> &subsets,
                              const FluentSet &superset1, const FluentSet &superset2) {
    FluentSet c;
...
void
get_operator_precondition(int op_index, FluentSet &pc) {
...
void
get_operator_effect(int op_index, FluentSet &eff) {
    Operator &op = g_operators[op_index];
...
void
HMLandmarks::print_pm_op(const PMOp &op) {
...
void
HMLandmarks::print_fluentset(const FluentSet &fs) {
    FluentSet::const_iterator it;
...
bool
HMLandmarks::possible_noop_set(const FluentSet &fs1, const FluentSet &fs2) {
    ...
    for (fs1it = fs1.begin(); fs1it != fs1.end(); ++fs1it) {
        for (fs2it = fs2.begin(); fs2it != fs2.end(); ++fs2it) {
            if (inconsistent_facts[fs1it->first][fs1it->second].find(*fs2it) !=
                inconsistent_facts[fs1it->first][fs1it->second].end()) {
                return false;
            }
        }
    }
...
bool
HMLandmarks::interesting(int var1, int val1, int var2, int val2) {
    // mutexes can always be safely pruned
    bool x = ((inconsistent_facts[var1][val1].find(std::make_pair(var2, val2)) == inconsistent_facts[var1][val1].end()) &&
              (!pm_fluent_pruning || g_causal_graph->have_common_ancestor(var1, var2)));
    return x;
}

HMLandmarks::HMLandmarks(Exploration *expl, int m)
    : LandmarksGraph(expl), m_(m) {
    pm_fluent_pruning = true;
...
HMLandmarks::init() {
    ...
    std::cout << "Using " << h_m_table_.size() << " P^m fluents. (pruning: "
    << pm_fluent_pruning << ")" << std::endl;
...
HMLandmarks::calc_achievers() {
    ...
    for (std::set<LandmarkNode *>::iterator it = nodes.begin();
         it != nodes.end(); it++) {
        LandmarkNode &lmn = **it;
        ...
        for (int i = 0; i < lmn.vars.size(); i++) {
            const std::vector<int> &ops =
                get_operators_including_eff(std::make_pair(lmn.vars[i],
                                                           lmn.vals[i]));
...
void
HMLandmarks::free_unneeded_memory() {
...
void
HMLandmarks::propagate_pm_fact(int factindex, bool newly_discovered,
                               TriggerSet &trigger) {
...
void
HMLandmarks::compute_h_m_landmarks() {
...
void
HMLandmarks::compute_noop_landmarks(int op_index, int noop_index,
...
void
HMLandmarks::add_lm_node(int set_index, bool goal) {
...
void
HMLandmarks::generate_landmarks() {
    int set_index;
    init();
