...
LandmarksGraphZhuGivan::proposition_layer
LandmarksGraphZhuGivan::build_relaxed_plan_graph_with_labels() const {
...
lm_set LandmarksGraphZhuGivan::apply_operator_and_propagate_labels(const Operator& op,
	...
	if(operator_cond_effect_fires(prepost[i].cond, current)) {
		...
		precond_label_union,
		union_of_condition_labels(prepost[i].cond, current)
		);	// NOTE: this equals precond_label_union, if prepost[i] is
