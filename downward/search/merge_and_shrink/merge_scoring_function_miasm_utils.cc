...
static unique_ptr<TransitionSystem> copy_and_shrink_ts(
    ...
    const ShrinkStrategy &shrink_strategy, int new_size, utils::LogProxy &log) {
    /*
...
unique_ptr<TransitionSystem> shrink_before_merge_externally(
    ...
    */
    pair<int, int> new_sizes = compute_shrink_sizes(
        original_ts1.get_size(), original_ts2.get_size(),
        max_states_before_merge, max_states);
    bool must_shrink_ts1 = original_ts1.get_size() >
                           min(new_sizes.first, shrink_threshold_before_merge);
    bool must_shrink_ts2 = original_ts2.get_size() >
                           min(new_sizes.second, shrink_threshold_before_merge);
    ...
    */
    ...
    if (must_shrink_ts1) {
        ts1 = copy_and_shrink_ts(
            original_ts1, fts.get_distances(index1), shrink_strategy,
            new_sizes.first, log);
    }
    unique_ptr<TransitionSystem> ts2 = nullptr;
    if (must_shrink_ts2) {
        ts2 = copy_and_shrink_ts(
            original_ts2, fts.get_distances(index2), shrink_strategy,
            new_sizes.second, log);
    }
    ...
    */
    return TransitionSystem::merge(
        fts.get_labels(), (ts1 ? *ts1 : original_ts1),
        (ts2 ? *ts2 : original_ts2), log);
}
}
