...
void State::heuristic_and_helpful_actions(int &cg_h, int &ff_h,
  ...
  if(g_ff_heuristic_replanning) {
    g_ff_heuristic->set_state(this);
    cg_h = g_ff_heuristic->get_ff_heuristic();
    ff_h = -2;
    // HACK! We put the FF heuristic estimate into the CG field,
    // so that the search algorithm need not be changed. This deserves
    // an award for extreme ugliness.
    return;
  }
